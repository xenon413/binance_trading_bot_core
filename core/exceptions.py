...
class BotError(Exception):
    def __init__(self, message:str, action:BotAction, init_wait:float=0, base_wait:float=0, max_retry:int=0):
        '''
        error with handle action
        retry/return/exit
        '''
        self.action = action
        ...
        self.is_critical = action is BotAction.EXIT or action is BotAction.RESTART
        self.formatted_traceback = traceback.format_stack() if self.is_critical else None
...
class BinanceError(IntEnum):
    ...
    INACTIVE_ACCOUNT = -4109
    INVALID_CLIENT_TRAN_ID_LEN = -4114
    DUPLICATED_CLIENT_TRAN_ID = -4115
    DUPLICATED_CLIENT_ORDER_ID = -4116
    STOP_ORDER_TRIGGERING = -4117
    ...
    ISOLATED_REJECT_WITH_JOINT_MARGIN_1 = -4183
    ...
    MODIFICATION_MIN_NOTIONAL = -5029
    INVALID_PRICE_MATCH = -5037
    UNSUPPORTED_ORDER_TYPE_PRICE_MATCH = -5038
    INVALID_SELF_TRADE_PREVENTION_MODE = -5039
    FUTURE_GOOD_TILL_DATE = -5040
    BBO_ORDER_REJECT = -5041
    EXISTING_PENDING_MODIFICATION = -5043

    @classmethod
    def get_by_code(cls, code:int):
        """
        Safe lookup that returns the Enum member or None if not found.
        """
        try:
            return cls(code)
        except ValueError:
    ...
    def get_error_config(self)->BotError:
        # format: { error_code: botErrr }
        return {
            # ---- RETRY ---
            -1003: BotError("TOO_MANY_REQUESTS", BotAction.RETRY, 60, 30, 3),

            0: BotError("SERVICE_UNAVAILSBLE", BotAction.RETRY, 10, 2, 5),

            -1021: BotError("INVALID_TIMESTAMP", BotAction.RETRY, 10, 2, 5),

            -2013:BotError("NO_SUCH_ORDER", BotAction.RETRY, 0, 0, 1),

            # ---- RETURN ----

            # this is not in use anymore but keep
            -2011: BotError("CANCEL_REJECTED", BotAction.RETURN),

            # forgot why this is here not sure if could be removed
            -4164: BotError("MIN_NOTIONAL", BotAction.RETURN),

            -5027:BotError("SAME_ORDER", BotAction.RETURN)
            # ---- RESTART ----

            # NOTE: Exit is default
        }.get(self.value, BotError(self.name, BotAction.EXIT))
